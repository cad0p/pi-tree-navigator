...
- *(navigate-tree)* `list` and `rewind` surface a `⚠ reflection bootstrap missing` warning with a recovery hint when the `AgentSession.prototype` patch isn't installed (typically only after a pi internals shape change). The hint suggests running `/reload` first (lighter-weight — re-runs the prototype patch on the current process) and `Restart pi` as the heavier-handed alternative.
...
- *(navigate-tree)* Re-anchoring is crash-safe: a mid-collision crash leaves the prior anchor intact (briefly visible alongside the new one in `list`) rather than an empty namespace, because `anchor` writes the new label before clearing the prior.
